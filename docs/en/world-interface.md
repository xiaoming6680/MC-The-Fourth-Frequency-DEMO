...
6. **Summon is an explicit press**, available only to those who have already handed one over, and only once every joined player present has finished escrowing. Pressing it commits atomically. On the screen it is a **large, centred, custom-drawn plate** rather than a vanilla button: it pulses while it is live (a 1.6-second cycle, far under the 3 Hz ceiling), whitens on hover, and goes flat grey when it is not.
