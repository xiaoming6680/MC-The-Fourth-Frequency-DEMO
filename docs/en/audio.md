...
There are two entry points: `config/thefourthfrequency.json`, and the **audio calibration page** of the first-run flow. Dragging the slider only changes the in-process config, so the neighbouring "Preview" is immediately at the new level. Preview plays the clean `terminal_boot_complete` cue; repeated presses replace the previous preview.
