...
| 4d | **First-boot graphics and ordinary direct entry** | First boot shows memory lamps, a scope and six states; ordinary opens show the page directly. The opening briefly lights a simple CRT-green hexagonal logo over about 1.5 seconds, using the same transparent asset as the mod icon, without a test card; the audio-to-disclosure page change retains its one-second frequency sweep. After confirmation the MC menu appears inside the terminal glass, the camera approaches smoothly, the frame leaves view and filters clear. Menu controls then work normally |
