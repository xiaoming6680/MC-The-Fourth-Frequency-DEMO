...
RC.6 已在独立验证副本中通过完整构建、861 项单元测试、99 项服务端 GameTest、完整 `all` 客户端回归，以及单独的 `notice-entry` 开屏回归。验证副本以 `3c77e8e` 为基线，仅包含本次开屏与音效改动；共享工作区其他任务尚未完成的异象、模型和桌面功能改动不在此发布包中。

开屏专项保留 9 张运行截图，检查实际 GPU 镜头滤镜的应用与清理；音频回归检查初次播放、持续声、刷新、静音及两类环境床的实际输出。242 份非音乐文件完整解码、电平、循环接缝和保留素材哈希验证通过。最终 JAR 验证覆盖 60 个 mixin 类与 22 个 Minecraft 注入目标。PCL 的 RC.6 已安装，复制后 SHA-256 与构建产物一致，旧 RC.5 移到工作区本地归档。

画面、电平和验证边界见 [RC.6 记录](../qa/audio_overhaul/rc6.md)。本轮未启动 PCL 正式实例进行人工听感与真实双客户端联机验收；这些项目仍列在文末。RC.5 历史结果见 [归档](../qa/audio_overhaul/rc5.md)。
