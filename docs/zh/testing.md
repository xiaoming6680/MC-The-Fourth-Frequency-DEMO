...
当前 RC.6 开屏简化修订以 `7810377` 为基线，独立通过 `build`（861 单元测试、99 服务端 GameTest 与 JAR 校验）及 `notice-entry` 真实客户端专项。开屏改为 1 秒亮起与 0.6 秒换页淡入淡出，原镜头入屏保留。专项检查双语布局、音量试听、切页门控、确认持久化以及实际 GPU 滤镜应用和清理；终端图形自检和音频资源未修改。

本次仅重跑受影响的开屏专项；完整 `all` 客户端与 242 音频解码的最近一次通过记录来自基线 `7810377`，见 [RC.6 历史记录](../qa/audio_overhaul/rc6.md)。本次运行截图及边界见 [开屏简化记录](../qa/audio_overhaul/entry-simplified.md)。PCL 已安装此修订并校验哈希；其他任务未完成的模型、异象和桌面改动保留在共享工作区，未混入测试发布包。正式实例人工听感与真实双客户端验收尚未完成。
